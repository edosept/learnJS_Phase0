...
function graduates (students) {
  ...
  if (students.length === 0) return tmp
  for (let i = 0; i < students.length; i++) {
    if (students[i].class in tmp === false) {
      tmp[students[i].class] = []
    }

    if (students[i].score > 75) {
      tmp[students[i].class].push({name: students[i].name, score: students[i].score})
    }
  }
