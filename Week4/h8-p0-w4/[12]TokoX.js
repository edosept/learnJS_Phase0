...
function countProfit(shoppers) {
  let listProduct = [['Sepatu Stacattu', 1500000, 10],
                     ['Baju Zoro', 500000, 2],
  ...
  if (shoppers.length > 0) {
    for (var i = 0; i < listProduct.length; i++) {
      ...
      for (var j = i; j < shoppers.length; j++) {
        if (shoppers[j].product == listProduct[i][0]) {
          if (shoppers[j].amount <= listProduct[i][2] ) {
              customersName.push(shoppers[j].name);
              profit += shoppers[j].amount * listProduct[i][1]
              listProduct[i].splice(2, 1 , listProduct[i][2] - shoppers[j].amount);
          }
        }
      }
      let shoppingList = {
        product: listProduct[i][0],
        shoppers: customersName,
        leftOver: listProduct[i][2],
        totalProfit: profit
      };
